...
export const generateSalesScript = baseProcedure
  .input(
    z.object({
      authToken: z.string(),
      companyId: z.number(),
      tone: z.enum(["professional", "friendly", "urgent"]).default("professional"),
      includeDecisionMakers: z.boolean().default(true),
    })
  )
...
- Accessibility Risk Score: ${company.riskScore}/100 (${company.riskScore >= 70 ? "HIGH RISK" : company.riskScore >= 40 ? "MEDIUM RISK" : "LOW RISK"})
...
${input.includeDecisionMakers && company.leads.length > 0 ? `Decision Makers:
